...
"use strict";
const game2048 = new (0, _gameClassJsDefault.default)();
 ...
 */ class Game {
   ...
   */ constructor(initialState = [
    ...
    renderBoard(b) {
        ...
        const prep = [];
        for (const elem of b)for (const e of elem)prep.push(e);
    ...
    moveLeft() {
        if (this.status === "playing") {
            ...
            if (JSON.stringify(newBoard) !== JSON.stringify(this.board)) {
                ...
                this.createCell();
            } else this.renderBoard(this.board);
        }
        this.canMove();
    }
    moveRight() {
    ...
    moveDown() {
        if (this.status === "playing") {
            ...
            for(let i = 0; i < 4; i++){
                let column = [];
                for(let j = 0; j < 4; j++)column.push(newBoard[j][i]);
                column.reverse();
                ...
                column = column.reduce((acum, element, index, arr)=>{
                    if (arr[index] === arr[index + 1]) {
                        ...
                        arr[index + 1] = 0;
                    } else if (element !== 0) acum.push(element);
                    return acum;
                }, []);
                while(column.length < 4)column.push(0);
                column.reverse();
                for(let j = 0; j < 4; j++)newBoard[j][i] = column[j];
            }
            if (JSON.stringify(newBoard) !== JSON.stringify(this.board)) {
                this.board = newBoard;
                this.renderBoard(this.board);
    ...
    moveUp() {
        if (this.status === "playing") {
            ...
            for(let i = 0; i < 4; i++){
                let column = [];
                for(let j = 0; j < 4; j++)if (newBoard[j][i] !== 0) column.push(newBoard[j][i]);
                column = column.reduce((acum, element, index, arr)=>{
                    if (arr[index] === arr[index + 1]) {
                        acum.push(element * 2);
                        this.updateScore(element * 2);
   ...
   */ restart() {
    ...
    updateScore(points) {
        if (points === 2048) this.status = "win";
        this.score += points;
    }
    canMove() {
